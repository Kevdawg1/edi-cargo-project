...
export default function EDICargoGenerator() {
  ...
  const generateEDI = async () => {
    try {
      const response = await axios.post('/generate-edi', cargoItems);
      setEdiOutput(response.data.edi);
    } catch (error) {
      alert('Error generating EDI');
    }
  };

  const parseEDI = async () => {
    try {
      const response = await axios.post('/parse-edi', { edi_text: ediInput });
          ...
          <div key={index} className="mb-6 border p-4 rounded-xl border-white/20">
            ...
            <label className="block mb-2">
              ...
              <input
                ...
                onChange={(e) => handleItemChange(index, 'master_bill_number', e.target.value)}
                className="w-full bg-gray-900 text-white p-2 rounded"
              />
            </label>
            <label className="block mb-2">
              ...
              <input
                ...
                onChange={(e) => handleItemChange(index, 'house_bill_number', e.target.value)}
                className="w-full bg-gray-900 text-white p-2 rounded"
              />
            </label>
          </div>
